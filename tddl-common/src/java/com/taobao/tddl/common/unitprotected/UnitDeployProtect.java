package com.taobao.tddl.common.unitprotected;
...
public class UnitDeployProtect {
	public static void unitDeployProtect(String sql,
			Map<Integer, ParameterContext> params) throws SQLException {
		try {
	...
	private static void unitDeployProtectWithCause(String sql,
		...
		if (c != null) {
			...
			if (c != null) {
				result = Router
						.isInCurrentUnit(Long.valueOf(String.valueOf(c)));
				eagleEyeRecord(result, c);
			}
		} else {
			// no unit protected
	...
	private static void eagleEyeRecord(boolean result, Object c)
...
//		clearUnitValidThreadLocal();
	...
	public static Object getValidFromHint(String sql,
			Map<Integer, ParameterContext> params) throws SQLException {
	...
	private static String tryRemoveUnitValidHintAndParameter(String sql,
			Map<Integer, ParameterContext> params) {
	...
	private static String replaceWithParams(String sql,
				...
				if (sql.charAt(i) == '?') {
					if (parameterSettings.get(parameters) == null) {
						throw new SQLException(
								"parameter size not match the place holder in sql,the parameter size is "
