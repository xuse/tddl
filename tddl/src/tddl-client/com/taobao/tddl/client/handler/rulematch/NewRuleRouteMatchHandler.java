...
/**
 * @description 为2.4.4引入的新规则体系提供的规则计算组件, 和RouteMatchHandler功能上并行,互不影响,
 *              与后者的区别在于规则计算引擎变为VirtualTableRuleMatcher, 并且规则封装类表为VirtualTable.
 * 
 *              规则引擎返回的数据结构未变,所以后续处理逻辑不变.
 * 
 *              新规则引擎使用需要在TDataSource的定义中设定useNewRule=ture
 *              属性,默认此属性为false,走老规则(>=2.3 && <=2.4.3),那么这个 handler不会被初始化
 * 
	...
	protected void match(DataBus dataBus) {
		...
		if (logicTableNames.size() == 1) {
			...
			if(spr.getInExpressionObjectList().size()>0&&needIdInGroup){
			    matcherResult = matcher.match(true,choicer, sqlParameters,
					rule);
			}else{
				matcherResult = matcher.match(false,choicer, sqlParameters,
						rule);
			}
			...
			for (String logicTableName : logicTableArray) {
				...
				if (rule == null) {
					...
					MatcherResult matcherResult = matcher.match(true,
