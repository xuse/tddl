...
import com.taobao.tddl.client.controller.SpringBasedRuleMatcherImpl;
...
/**
 * @description 此handler主要功能就是调用规则引擎计算 库后缀和表后缀,最终目的
 *              是拿到 真实库与真实表 这样一个对应关系.
 *               
 *              2.4.4(包含)之前版本TDDL,规则引擎在计算完所有库后再计算表,后续
 *              将加入算完一个库马上算表的方式,减少中间步骤.
 *              
 *              此handler还进行同句sql中多表的计算, 主要目的就是应对非单表join,但是只支持
 *              单库多表join,对于跨库多表join暂不支持(直接抛错).
 * @author <a href="junyu@taobao.com">junyu</a>
 * @version 2.4.4
 * @since 1.6
 * @date 2010-09-08下午06:33:32
 */
public class RouteMatchHandler extends AbstractHandler {
	public static final String HANDLER_NAME = "RouteMatchHandler";
	private final Log log = LogFactory.getLog(RouteMatchHandler.class);
	 ...
	 * @param dataBus
	 */
	protected void match(DataBus dataBus) throws SQLException{
		...
		if (logicTableNames.size() == 1) {
			...
			if(spr.getInExpressionObjectList().size()>0&&needIdInGroup){
				matcherResult = matcher.match(newTypeRuleCalculate,true,choicer, sqlParameters,
						rule);
			}else{
				matcherResult = matcher.match(newTypeRuleCalculate,false,choicer, sqlParameters,
						rule);
			}
			...
			setResult(databaseExecutionContexts, matcherResult, null,
					...
					rule.getUniqueColumns(), needIdInGroup, completeDistinct,
					runtime);
		} else {
			/*
			...
			for (String logicTableName : logicTableArray) {
				...
				if (rule == null) {
					...
					MatcherResult matcherResult = matcher.match(newTypeRuleCalculate,true,
							sqlParserResult.getComparativeMapChoicer(),
							sqlParameters, rule);
	...
	private List<DatabaseExecutionContext> convertToDatabaseExecutionContext(
			String logicTableName, List<TargetDB> targetDBs) throws SQLException{
		...
		for (TargetDB targetDB : targetDBs) {
			...
			if(null==tableSet){
				...
				sb.append("规则计算后，目标库");
				sb.append(targetDB.getDbIndex());
			    sb.append("没有可执行表，请检查sql参数");
