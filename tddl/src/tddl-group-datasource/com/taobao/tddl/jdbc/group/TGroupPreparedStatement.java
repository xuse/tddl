 ...
 * 
 */
public class TGroupPreparedStatement extends TGroupStatement implements
		TPreparedStatement {

	private static final Log log = LogFactory
			.getLog(TGroupPreparedStatement.class);

	private String sql;

	public TGroupPreparedStatement(TGroupDataSource tGroupDataSource,
	...
	private PreparedStatement createPreparedStatementInternal(Connection conn,
		...
		if (autoGeneratedKeys != -1) {
			...
			ps = conn.prepareStatement(sql, this.resultSetType,
					this.resultSetConcurrency, resultSetHoldability);
		}
		setBaseStatement(ps);
	...
	public ResultSet executeQuery() throws SQLException {
		...
		boolean gotoRead = SqlType.SELECT.equals(SqlTypeParser.getSqlType(sql))
		...
		if (conn != null) {
			...
			return tGroupDataSource.getDBSelector(gotoRead).tryExecute(
					executeQueryTryer, retryingTimes, sql, dataSourceIndex);
		}
	}
	...
	public int executeUpdate() throws SQLException {
		...
		if (conn != null) {
			...
			// #bug 2011-10-28,modify by junyu,updateCount not set,fixed
			int updateCount = tGroupDataSource.getDBSelector(false).tryExecute(
					null, executeUpdateTryer, retryingTimes, sql,
					dataSourceIndex);
			super.updateCount = updateCount;
			return updateCount;
		}
	...
	private DataSourceTryer<Integer> executeUpdateTryer = new AbstractDataSourceTryer<Integer>() {
		public Integer tryOnDataSource(DataSourceWrapper dsw, Object... args)
				throws SQLException {
		...
		try {
			...
			if (conn != null) {
				...
				return tGroupDataSource.getDBSelector(false).tryExecute(null,
						executeBatchTryer, retryingTimes, dataSourceIndex);
			}
		} finally {
			if (pstArgs != null)
				pstArgs.clear();
	...
	private DataSourceTryer<int[]> executeBatchTryer = new AbstractDataSourceTryer<int[]>() {
		public int[] tryOnDataSource(DataSourceWrapper dsw, Object... args)
				throws SQLException {
	...
	private static void setBatchParameters(PreparedStatement ps,
			Collection<ParameterContext> batchedParameters) throws SQLException {
