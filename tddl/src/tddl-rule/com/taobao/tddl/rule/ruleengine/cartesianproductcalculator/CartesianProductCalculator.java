...
public class CartesianProductCalculator implements Parent,
	...
	/**
	 * samplingField在内存里是被复用的，因为他本身只是用于值的传递的，而且是在单线程里面被使用。 他的父class
	 * CartesianProductCalculator本身是在每次计算笛卡尔积的时候都会新建的，因此里面的类 是不需要关注线程安全的
	 * 
	 */
	...
	public CartesianProductCalculator(Map<String, Set<Object>> enumeratedMap) {
		...
		samplingFieldToBeReturned = new SamplingField(columnList,
				enumeratedMap.size());
		...
		if (!enumeratedValuesSetOrderByColumnList.isEmpty()) {
			for (Set<Object> set : enumeratedValuesSetOrderByColumnList) {
				// parent
				parentProductor = new CartesianProductCalculatorElement(null,
						set);
				if (isFirst) {
