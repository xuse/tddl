...
public class SimpleLogicTable extends LogicTable{
	...
	/**
	 * 大部分业务应用可以使用这个tableMapProvider直接进行配置，搞定分表
	 */
	private SimpleTableMapProvider simpleTableMapProvider = new SimpleTableMapProvider();
	/**
	 * 全局简单分表模式
	 */
	TableMapProvider globalCustomTableMapProvider;
	...
	public void init(boolean invokeBySpring) {
		...
		if(superClassTableMapProviderIsNull()){
			...
			}else if(globalCustomTableMapProvider != null) {
				setTableMapProvider(globalCustomTableMapProvider);
		    }
		}
		...
		if(superClassDatabaseRuleIsNull()){
			RuleChain rc = RuleUtils.getRuleChainByRuleStringList(databaseRuleStringList, GroovyListRuleEngine.class,isDatabase,extraPackagesStr);
			
			super.listResultRule = rc;
		}
		
		if(transmitterTableRuleIsNull()){
			RuleChain rc = RuleUtils.getRuleChainByRuleStringList(tableRuleStringList, GroovyListRuleEngine.class,!isDatabase,extraPackagesStr);
	...
	public TableMapProvider getGlobalCustomTableMapProvider() {
		return globalCustomTableMapProvider;
	}

	public void setGlobalCustomTableMapProvider(
			TableMapProvider globalCustomTableMapProvider) {
		this.globalCustomTableMapProvider = globalCustomTableMapProvider;
	}
