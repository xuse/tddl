...
public final class ConnRestrictEntry {
	...
	public String toString() {
		return "ConnRestrictEntry: " + Arrays.toString(keys) + " " + hashSize
				+ ", " + limits;
	}
	 ...
	 */
	public static ConnRestrictEntry parseEntry(String key, String value,
			int maxPoolSize) {
		...
		if (KEY_WILDCARD.equals(key)) {
			...
			if (find >= 0) {
				...
				if (countStr.isEmpty()) {
					...
					if (numberStr.endsWith(PERCENT)) {
						numberStr = StringUtil.substringBefore(numberStr,
								PERCENT);
						if (StringUtil.isNumeric(numberStr)) {
							ConnRestrictEntry connRestrictEntry = new ConnRestrictEntry(
									new String[] { key }, 1, maxPoolSize
											* Integer.valueOf(numberStr) / 100);
							return connRestrictEntry;
						}
					} else if (StringUtil.isNumeric(numberStr)) {
						ConnRestrictEntry connRestrictEntry = new ConnRestrictEntry(
								new String[] { key }, 1,
								Integer.valueOf(numberStr));
						return connRestrictEntry;
					}
				} else if (StringUtil.isNumeric(countStr)) {
					// Parse "*:count,number"
					if (numberStr.endsWith(PERCENT)) {
						numberStr = StringUtil.substringBefore(numberStr,
								PERCENT);
						if (StringUtil.isNumeric(numberStr)) {
							ConnRestrictEntry connRestrictEntry = new ConnRestrictEntry(
									new String[] { key },
									Integer.valueOf(countStr), maxPoolSize
											* Integer.valueOf(numberStr) / 100);
							return connRestrictEntry;
						}
					} else if (StringUtil.isNumeric(numberStr)) {
				...
				if (value.endsWith(PERCENT)) {
					String numberStr = StringUtil.substringBefore(value,
							PERCENT);
					if (StringUtil.isNumeric(numberStr)) {
						ConnRestrictEntry connRestrictEntry = new ConnRestrictEntry(
								new String[] { key }, 1, maxPoolSize
					...
					ConnRestrictEntry connRestrictEntry = new ConnRestrictEntry(
							new String[] { key }, 1, Integer.valueOf(value));
					return connRestrictEntry;
				}
			}
			...
			// Parse "~:number"
			if (value.endsWith(PERCENT)) {
				String numberStr = StringUtil.substringBefore(value, PERCENT);
				if (StringUtil.isNumeric(numberStr)) {
					ConnRestrictEntry connRestrictEntry = new ConnRestrictEntry(
							new String[] { key }, 1, maxPoolSize
									* Integer.valueOf(numberStr) / 100);
