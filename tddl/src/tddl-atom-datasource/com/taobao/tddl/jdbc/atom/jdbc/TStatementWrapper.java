...
import com.taobao.tddl.common.eagleeye.EagleeyeHelper;
...
import com.taobao.tddl.common.util.SqlTypeParser;
/**
 * Statement 包装类
 * @author shenxun
 *
...
public class TStatementWrapper implements TStatement {
	...
	void close(boolean removeThis) throws SQLException {
        ...
        try {
            ...
            this.targetStatement = null; //端口与物理statement的引用，底下可能会有ps cache，导致节点无法被gc
            if (removeThis){
                // 关闭之后，移除
...
//			throw e;
	...
	protected void startRpc(String sqlType){
		EagleeyeHelper.startRpc(datasourceWrapper.runTimeConf.getIp(),
				...
				datasourceWrapper.runTimeConf.getDbName(), sqlType);
	}
...
//			throw e;
			...
			decreaseConcurrentWrite();
			recordSql("batch sql without distinguish the sql or parameters",
					System.currentTimeMillis() - time0, e0);
		}
	}
