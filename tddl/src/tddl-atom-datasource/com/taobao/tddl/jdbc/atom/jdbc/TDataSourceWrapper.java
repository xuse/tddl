...
import com.taobao.tddl.common.client.jdbc.sorter.ExceptionSorter;
import com.taobao.tddl.common.client.jdbc.sorter.MySQLExceptionSorter;
import com.taobao.tddl.common.client.jdbc.sorter.OracleExceptionSorter;
...
//	private volatile SmoothValve smoothValve = new SmoothValve(20);
	...
	private volatile CountPunisher timeOutPunisher = new CountPunisher(new SmoothValve(0), 3000, 300);//3秒钟之内超时300次则惩罚，不可能的阀值，相当于关闭了
...
//			throw e;
			...
			throw new SQLException(
					"get connection failed,dbKey(if null,with dbName) is "
							+ (connectionProperties.datasourceName != null ? connectionProperties.datasourceName
									: this.runTimeConf.getDbName()), e);
		}
	}
	...
	private Connection getConnection0(String username, String password) throws SQLException {
		...
		try {
			...
			if (connRestrictor != null) {
				connRestrictSlot = connRestrictor.doRestrict(
						runTimeConf.getBlockingTimeout());
			}
			tconnectionWrapper = new TConnectionWrapper(
					getConnectionByTargetDataSource(username, password), 
					connRestrictSlot, this);
		} catch (SQLException e) {
